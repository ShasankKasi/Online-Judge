...
export default function Admin() {
  ...
  async function handleSubmit(e){
   ...
   try{
       const response=await axios.post("http://localhost:8000/admin",{title,description,testcases})
        ...
        <div>
          ...
          <div>
            <textarea
              ...
              value={input[1]}
              onChange={(e)=>setInput([input[0],e.target.value, ...input.slice(2)])}
            ></textarea>
            <textarea
              placeholder="Enter the output2"
              value={output[1]}
              onChange={(e)=>setOutput([output[0],e.target.value, ...output.slice(2)])}
            ></textarea>
          </div>
          <div>
            <textarea
              ...
              value={input[2]}
              onChange={(e)=>setInput([...input.slice(0,2),e.target.value, ...input.slice(3)])}
            ></textarea>
            <textarea
              placeholder="Enter the Output3"
              value={output[2]}
              onChange={(e)=>setOutput([...output.slice(0,2),e.target.value, ...output.slice(3)])}
            ></textarea>
          </div>
          <div>
            <textarea
              ...
              value={input[3]}
              onChange={(e)=>setInput([...input.slice(0,3),e.target.value,input[4]])}
            ></textarea>
            <textarea
              placeholder="Enter the Output 4"
              value={output[3]}
              onChange={(e)=>setOutput([...output.slice(0,3),e.target.value,output[4]])}
            ></textarea>
          </div>
          <div>
            <textarea
            ...
            <textarea
              ...
              value={output[4]}
              onChange={(e)=>setOutput([...output.slice(0,4),e.target.value])}
